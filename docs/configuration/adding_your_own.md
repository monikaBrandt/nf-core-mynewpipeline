# nf-core/mynewpipeline: Configuration for other clusters
...
Nextflow has [excellent integration](https://www.nextflow.io/docs/latest/docker.html) with Docker, and beyond installing the two tools, not much else is required - nextflow will automatically fetch the [nfcore/mynewpipeline](https://hub.docker.com/r/nfcore/mynewpipeline/) image that we have created and is hosted at dockerhub at run time.

To add docker support to your own config file, add the following:

```nextflow
docker.enabled = true
process.container = "nfcore/mynewpipeline"
```
...
process.container = "shub://nf-core/mynewpipeline"
```
...
```nextflow
singularity.enabled = true
process.container = "/path/to/nf-core-mynewpipeline.simg"
```
